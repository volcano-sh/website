...
使用指导请参考：[Capacity Plugin User Guide](/docs/Plugins/user_guide_how_to_use_capacity_plugin).
...
`capacity`在v1.10.0版本中作为了默认的队列管理插件，`capacity`与`proportion`插件互相冲突，当升级到v1.10.0后，你需要再设置队列的`deserved`字段，以保证队列功能正常工作，具体的使用说明请参考：[Capacity Plugin User Guide](/docs/Plugins/user_guide_how_to_use_capacity_plugin).
