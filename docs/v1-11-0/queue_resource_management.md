...
[Queue](/docs/queue) is one of the core concepts in Volcano, designed to support resource allocation and task scheduling in multi-tenant scenarios. Through queues, users can implement multi-tenant resource allocation, task priority control, resource preemption and reclamation, significantly improving cluster resource utilization and task scheduling efficiency.
...
### 1. Flexible Resource Configuration
   ...
   * Provides three-level resource configuration mechanism:
      ...
      * guarantee: Reserved resource amount (reserved resources can only be used by this queue, other queues cannot use them)
      ...
      >
      > 1. When configuring three-level resources, follow: guarantee \<=lt;= deserverd \<=lt;= capability;
...
### 2. Hierarchical Queue Management
   ...
   * Supports [hierarchical queue](/docs/hierarchical_queue) structure
...
The capacity plugin enables quota control through precise resource configuration. Combined with [hierarchical queues](/docs/hierarchical_queue), it can achieve more fine-grained multi-tenant resource allocation and facilitates big data workload migration to Kubernetes clusters.
