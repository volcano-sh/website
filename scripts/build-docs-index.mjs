// Chunk docs/*.md into netlify/functions/ask-ai/docs-index.json for Ask AI.
...
function docUrl(relPath, frontmatter) {
  if (frontmatter.slug) {
    return `/docs/${String(frontmatter.slug).replace(/^\//, "")}`;
  }
  return `/docs/${relPath.replace(/\.mdx?$/, "").replace(/\\/g, "/")}`;
}
...
fs.writeFileSync(OUT_FILE, JSON.stringify({ generatedAt: new Date().toISOString(), chunks }));
console.log(`indexed ${chunks.length} chunks from ${files.length} docs`);
